...
def plot(y_array, config=None):
    ...
    for i in range(width_x_axis):
        if ((i % tick_distance == 0 and (i + tick_distance) < width_x_axis) or i == (width_x_axis - 1)):
            ticks += "┬"
        else:
            ticks += "─"
    ...
    if width_x_axis <= tick_distance:
        tick_labels += str(last_tick_value).rjust(width_x_axis - (len(tick_labels) - left_margin + 1))
    else:
        for i in range(width_x_axis):
            # BPER: fix the x Axis
            tick_value = x_array[round(i/width_x_axis * original_width)]
    ...
    title = f"{' ' * (left_margin + (width_x_axis - len(config.get('title', '')))//2)}{config.get('title', '')}\n" if 'title' in config else ''
    ...
    if 'y_label' in config or isinstance(config.get('line_labels', None), list):
        ...
        if isinstance(config.get('line_labels', None), list):
            ...
            for i in range(min(len(y_array), len(config.get('line_labels')))):
                color = config.get('colors', [])[i] if isinstance(config.get('colors', None), list) else 'default'
                legend += f"    {color}─── {config['line_labels'][i]}{asciichart.reset}"
            y_label += ' ' * (full_width - 1 - len(strip_ansi(legend)) - len(strip_ansi(y_label))) + legend
        y_label += f"\n{'╷'.rjust(left_margin)}\n"
